...
fixed4 PSMain(SHADERDATA ps) : SV_TARGET {
    fixed4 col = tex2D(_TextureMap, ps.uv * _TileAmount);
    return lerp(col * float4(_ShadowBrightness, _ShadowBrightness, _ShadowBrightness, 1), col, step(0.5, SHADOW_ATTENUATION(ps))) * _Color;
}
